...
我们可以将我们的管道改为同时执行两个mission实例，每个都从同样的输入channel读取数据。我们还引入新函数，merge，来扇入所有的结果：
```go
func main() {
    p := gen(2, 3)
...
但是往一个已经关闭的channel输出会产生异常（panic），所以一定要保证所有数据发送完成后再执行关闭。sync.WaitGroup类型提供了方便的方法，来保证这种同步：
```go
