...
func main() {
	...
	fmt.Println("Width:", r.width, "* Length:",
		r.length, "= Area:", r.width*r.length)
}
```
输出结果为:
...
func main() {
	...
	r.length = 200
	fmt.Println(*r)
	fmt.Println("Double Width:", r.width, "Double Length:", r.length,
		"Double Area:", r.double_area())
	fmt.Println(*r)
}
```
* 结构体特性 
...
```
由于匿名嵌入的特性，我们可以直接访问子属性而不需要给出完整的路径，这样就允许我们去重载通过匿名字段继承的一些字段，当然如果我们想访问重载后对应匿名类型里面的字段，可以通过匿名字段名来访问。
```go
var w Wheel
